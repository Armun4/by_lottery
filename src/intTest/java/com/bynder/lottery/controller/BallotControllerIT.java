...
public class BallotControllerIT extends BaseIT {
  ...
  private final TypeRef<List<BallotResponse>> ballotListType = new TypeRef<>() {};
  ...
  @Test
  void shouldCreateLotteryIfLotteryNotFound() {
    ...
    Lottery createdLottery =
        lotteryRepository
            .getCurrentLottery(LocalDate.ofInstant(currentStartTime, ZoneOffset.UTC))
            .get();

    BallotResponse expected =
        BallotResponse.builder()
            .lotteryId(createdLottery.getId())
            .participantId(savedParticipant.getId())
            .build();
